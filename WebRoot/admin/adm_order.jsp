...
<tr></tr>
<tr>
<% if (!mesg.equals("")) out.println("<font color=black>" + mesg + "</font><br>");%>
		...
		for(int i=0 ; i<shop.getAllorder().size(); i++){
		  ...
		  <tr>
			...
			<% if (Ident.getIsSales())
					...
					out.print("未发货");
			%></td>
            <td align="center"><a href="order_detail.jsp?indentid=<%= Ident.getOrderId() %>">详细情况</a>&nbsp;<a href="adm_order.jsp?action=del&indentid=<%= Ident.getId()%>&page=<%= shop.getPage() %>" onclick="return(confirm('你真的要删除吗？'))">删除</a></td>
          </tr>
<%		}
  }
		 ...
		 <table width="778" border="0" cellspacing="1" cellpadding="1" bgcolor="#E4EDFB"  bordercolor=#CC0000 align="center">
          <tr>
            <td align="right">当前页第<%= shop.getPage() %>页　<a href="adm_order.jsp">首页</a>&nbsp;
              ...
              <% } %>
              <% if (shop.getPage()<shop.getPageCount()) {%>
