...
%>
          <tr>
			...
			<td><%= userinfo.getUserPoint()%></td>
            <td align=center><a href="user_detail.jsp?userid=<%= userinfo.getId() %>">详细资料</a>&nbsp;
            <a href="user_modify.jsp?userid=<%= userinfo.getId() %>">修改</a>&nbsp;
            <a href="adm_user.jsp?userid=<%= userinfo.getId()%>&page=<%= user.getPage()%>&action=del" onclick="return(confirm('你真的要删除这个用户?'))">删除</a></td>
          </tr>
<%	}
}
		...
		<table align="center" width="778" border="0" cellspacing="1" cellpadding="1"  bgcolor="#E4EDFB">
          <tr>
            <td align="right">当前页第<%= user.getPage() %>页　<a href="adm_user.jsp">首页</a>&nbsp;
              ...
              <% } %>
              <% if (user.getPage()<user.getPageCount()) {%>
