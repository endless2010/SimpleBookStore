...
<table width="778">
  <tr>
     ...
     <td width="600">
          <p align="center"><b><font color="#0000FF">网上书店图书<%= classname %>列表
	...
	if (book_list.getBooklist().size()>0 ){
		for (int i=0;i<book_list.getBooklist().size();i++){
          ...
          <td>  <a href="detail.jsp?bookid=<%=bk.getId()%>"><%= bk.getBookName() %></a></td>
            ...
            <td align="center"><%= bk.getPrice() %>元</td>
            <td align="center"><%= bk.getMemberPrice() %>元</td>
            <td align="center"><a href="buy.jsp?bookid=<%=bk.getId()%>" >购买</a>&nbsp;
			<a href="detail.jsp?bookid=<%=bk.getId() %>" >详细资料</a></td>
          </tr>
<%		}
	}else {
		if (keyword.equals("")){
          ...
          <tr>
            <td align="center" colspan=6>&nbsp;暂时没有此类图书资料
            <br>
           </td>
          </tr>
<% } %>
